...
Die Workbooks zum Transport Tendering folgen derselben didaktischen Logik. Alle Werte sind fiktiv und wurden fuer Lehrzwecke generiert.

## Workbook-Blaetter

### Shipment_Execution

Haupttabelle der ausgefuehrten Shipments.

| Feld | Bedeutung |
| --- | --- |
| shipment_id | Eindeutige generierte Shipment-Kennung. Doppelte Referenzen kommen in ausgewaehlten Zeilen absichtlich vor. |
| shipment_date | Datum, an dem das Shipment an die Transportplanung uebergeben wurde. |
...
| actual_carrier_id | Carrier, der das Shipment ausgefuehrt hat. Dieser kann vom geplanten Carrier abweichen. |
...
| contracted_rate_eur | Vertragliche Frachtrate fuer geplanten Carrier und Lane. |
| actual_freight_cost_eur | Tatsaechlich gezahlte Frachtkosten. |
| spot_premium_eur | Differenz zwischen tatsaechlichen Kosten und vertraglicher Rate, wenn Spot- oder Premiumkapazitaet genutzt wurde. |
| fuel_surcharge_eur | Fuel Surcharge fuer das Shipment. |
...
| actual_delivery_date | Tatsaechliches Lieferdatum, sofern erfasst. Einige Zeilen enthalten absichtlich fehlende Werte. |
| delay_hours | Lieferverzoegerung in Stunden gegenueber Plan. |
...
| co2e_kg | Geschaetztes Transport-CO2e in Kilogramm. |
| co2_target_kg | Shipment-spezifisches CO2e-Ziel. |
| co2_target_missed_flag | Kennzeichnet, ob das geschaetzte CO2e das Ziel ueberschritten hat. |
| exception_code | Systemhinweis auf ausgewaehlte Anomalien. Leer bedeutet nicht automatisch korrekt. |

### Spot_Quotes

Spotmarktangebote, verknuepft mit Tenderfehlern, dringender Nachfrage oder manuellen Spotkaeufen.

| Feld | Bedeutung |
| --- | --- |
| spot_quote_id | Eindeutige generierte Spot-Quote-Kennung. |
| shipment_id | Zugehoerige Shipment-Kennung, sofern vorhanden. |
...
| selected_flag | Kennzeichnet, ob dieses Angebot ausgewaehlt wurde. |
...
| estimated_co2e_kg | Geschaetztes CO2e des angebotenen Transports. |
| acceptance_status | Offered, Accepted, Declined, Expired oder No Capacity. |
| quote_rank_by_price | Preisrang innerhalb des Quote Sets, soweit verfuegbar. |
...
| capacity_commitment_shipments_per_week | Woechentliche Kapazitaetszusage. |
...
| committed_co2e_kg_per_ton_km | Vom Carrier zugesagte Emissionsintensitaet. |
...
| commercial_score | Tender Score fuer Preiswettbewerbsfaehigkeit. |
| service_score | Tender Score fuer Serviceversprechen und Historie. |
| sustainability_score | Tender Score fuer Emissionen und Equipmentprofil. |
...
| tender_id | Zugehoerige Tenderprozesskennung. |
| lane_id | Transport-Lane-Kennung. |
| carrier_id | Vergebene Carrier-Kennung. |
| award_rank | Primary, Secondary, Backup oder Spot Framework. |
| awarded_share_pct | Geplanter Anteil des Lane-Volumens fuer den Carrier. |
| contracted_rate_eur | Vertragliche Frachtrate. |
| committed_capacity_per_week | Woechentliche Kapazitaetszusage des Carriers. |
| target_acceptance_pct | Zielwert fuer Tender Acceptance. |
| target_otd_pct | Zielwert fuer On-Time Delivery. |
| target_co2e_kg_per_ton_km | Zielwert fuer Emissionsintensitaet. |
...
| sustainability_clause_flag | Kennzeichnet, ob der Vertrag eine Nachhaltigkeitsklausel enthaelt. |
...
Woechentliche Carrier Scorecard nach Carrier und Lane Group.
...
| on_time_rate | Anteil der ausgefuehrten Shipments, die puenktlich geliefert wurden. |
| avg_delay_hours | Durchschnittliche Verspaetung in Stunden. |
| missed_appointments | Anzahl Shipments mit verpasstem Liefertermin. |
| claims_count | Anzahl Schadens- oder Servicefaelle. |
| contracted_cost_eur | Vertraglicher Kostenbaseline der tendered Shipments. |
| actual_cost_eur | Tatsaechlich gezahlte Kosten fuer zugeordnete Shipments. |
| spot_substitution_cost_eur | Spot- oder Premiumkosten, die Tenderfehlern zugeordnet werden. |
| co2e_kg | Geschaetztes tatsaechliches CO2e. |
| co2_target_kg | CO2e-Ziel. |
| co2_target_met_flag | Kennzeichnet, ob die woechentliche Scorecard das Emissionsziel erreicht hat. |
| exception_code | Systemhinweis auf ausgewaehlte Anomalien. Leer bedeutet nicht automatisch korrekt. |
...
| shipment_id | Zugehoerige Shipment-Kennung. |
...
| delay_hours | Verspaetungsstunden, soweit relevant. |
| claim_amount_eur | Geforderter oder geschaetzter Betrag. |
...
| rate_index | Relative Preiswettbewerbsfaehigkeit. Niedrigere Werte zeigen guenstigere Carrier. |
| service_index | Relative Servicezuverlaessigkeit. |
| acceptance_index | Relative Tender-Acceptance-Zuverlaessigkeit. |
| co2_index | Relative Emissionsintensitaet. Niedrigere Werte zeigen niedrigere Emissionen. |
| fleet_euro6_share | Anteil Euro-6- oder vergleichbarer Flotte. |
| alternative_fuel_share | Anteil alternativer Kraftstoffe, elektrischer oder intermodalfaehiger Ausstattung. |
| capacity_tightness | Relativer Kapazitaetsengpassindikator. |
| sustainability_score | Carrier Sustainability Score. |

### Lane_Master

Lane-Referenzdaten mit Ursprung, Ziel, Lane Group, Distanz, Komplexitaet, Basiskosten, Volumen und CO2-Zielattributen.
...
Kundensegmentbezogene Service-Sensitivitaet und Penalty-Annahmen.

### Cost_Parameters

Geschaetzte Kosten fuer Spot Premiums, verpasste Termine, Kundenstrafen, Claims, CO2-Abweichungen und Tenderadministration.

### Emissions_Factors

Modusbezogene Emissionsannahmen, die fuer synthetische CO2-Berechnungen genutzt wurden.
...
| --- | --- | --- |
| Accepted tender | Vertraglicher Carrier hat den Tender akzeptiert und das Shipment ausgefuehrt. | Basis der Carrier Performance. |
| Rejected tender | Vertraglicher Carrier hat den Tender nicht akzeptiert. | Analyse von Tender Waterfall und Kapazitaetsrisiko. |
| Brokered spot | Shipment wurde ueber Spotmarkt oder Brokerkapazitaet bewegt. | Diagnose versteckter Kosten und Resilienz. |
...
| CO2 target miss | Shipment-Emissionen lagen ueber Ziel. | Nachhaltigkeits-Trade-off-Analyse. |

## Analytische Hinweise

- Ein Low-Rate-Carrier ist nicht automatisch schlecht, wenn er auf dem richtigen Lane-Typ gut performt.
- Eine hohe On-Time Rate kann haeufige Tender Rejections verdecken, wenn nur ausgefuehrte Shipments betrachtet werden.
- Spotmarktaufwand sollte soweit moeglich auf den abgelehnten oder fehlgeschlagenen Contract Carrier zurueckgefuehrt werden.
